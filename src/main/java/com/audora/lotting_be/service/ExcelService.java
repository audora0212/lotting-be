...
public class ExcelService {
    ...
    public void fillRegFormat(File tempFile, List<Customer> customers) throws IOException {
        try (FileInputStream fis = new FileInputStream(tempFile);
            ...
            for (Customer customer : customers) {
                ...
                // Column DW (126): MGM 수수료 (숫자 그대로)
                Cell cellDW = row.getCell(126);
                if (cellDW == null) cellDW = row.createCell(126);
                cellDW.setCellType(CellType.NUMERIC);
                cellDW.setCellValue((customer.getMgm() != null && customer.getMgm().getMgmfee() != null)
                        ? customer.getMgm().getMgmfee() : 0);
    ...
    /**
     * 수정된 processRegExcelFile:
     * - 엑셀 파일의 첫번째 시트에서 4번째 행(인덱스 3)부터 시작하여,
     *   A열(관리번호)이 공백값일 때까지 각 행을 읽어 고객 객체를 생성 후 DB에 저장.
     */
    public void processRegExcelFile(MultipartFile file) throws IOException {
        try (InputStream is = file.getInputStream();
            ...
            while (true) {
                ...
                // A열(인덱스 0)의 값이 공백이면 종료
                String colAval = formatter.formatCellValue(row.getCell(0), evaluator);
                if (colAval == null || colAval.trim().isEmpty()) {
                    break;
                }

                // 고객 및 하위 객체 초기화 (기존 로직 복사)
                ...
                if (!colDD_final.isEmpty()) {
                    try {
                        ...
                        refundAmount = 0; // 숫자 파싱 실패 시 0으로 처리
                    }
                }
                ...
                if (!colDE_final.isEmpty()) {
                    try {
                        amountSum = Long.parseLong(colDE_final.replaceAll("[^0-9-]+", ""));
                    } catch (NumberFormatException e) {
                        amountSum = 0; // 숫자 파싱 실패 시 0으로 처리
                    }
                }
